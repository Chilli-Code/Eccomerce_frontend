...
const ProductCard = ({ product, onView, onAdd }) => {
  ...
  const handleAdd = (e) => {
    e.stopPropagation();
    setAdded(true);
    onAdd?.(product); // ← pasa el producto completo
  ...
  return (
    <div className="flex flex-col group cursor-pointer" onClick={() => onView?.()}>
      <div
        className="relative w-full rounded-3xl mb-3 overflow-hidden"
        style={{ backgroundColor: product.bg || "#f5f5f5", aspectRatio: "4 / 5" }}
      >
        <img
          src={product.img || product.images?.[0] || "https://placehold.co/600x750?text=Sin+imagen"}
          alt={product.name}
        ...
        <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-3 z-10">
          <button
            onClick={handleAdd}
            className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest px-7 py-3 rounded-full transition-all duration-300 ${
              added ? "bg-green-500 text-white scale-95" : "bg-white text-zinc-900 hover:bg-zinc-100"
            }`}
          >
      ...
      <p className="text-xs text-zinc-400 tracking-widest uppercase mb-0.5">
        {product.brand || ""}
      </p>
